...
class StackedAutoEncoder:
    ...
    def fit(self, train_x, val_x=None, save_model=False):
        ...
        for i in range(self.depth):
            ...
            if self.noise is None:
                train_x, val_x = self._run(data_x=train_x, val_x=val_x, activation=self.activations[i],
                                           data_x_=train_x, val_x_=val_x, hidden_dim=self.dims[i],
                                           epoch=self.epoch[i], loss=self.loss, batch_size=self.batch_size,
                                           lr=self.lr, print_step=self.print_step, depth=i + 1, load_model=False,
                                           save_model=save_model)
            else:
                temp_train = np.copy(train_x)
                temp_val = None if val_x is None else np.copy(val_x)
                train_x, val_x = self._run(data_x=self._add_noise(temp_train), val_x=self._add_noise(temp_val),
                                           ...
                                           depth=i + 1, load_model=False, save_model=save_model)
        if save_model:
            ...
            for i in range(self.depth):
                ...
                if self.noise is None:
                    ...
                    train_x, val_x = self._run(data_x=self._add_noise(temp_train), val_x=self._add_noise(temp_val),
                                               ...
                                               print_step=self.print_step, depth=i + 1, load_model=True,
                                               save_model=True)
        else:
            # model must be trained once, before partial_fit
            self.fit(train_x, val_x, save_model=True)
    ...
    def _run(self, data_x, data_x_, val_x, val_x_, hidden_dim, activation, loss, lr,
             print_step, epoch, batch_size=100, depth=0, load_model=False, save_model=False):
        ...
        with tf.Session() as sess:
            if load_model:
                ...
                encode = {'weights': graph.get_tensor_by_name("enc_weights:0"),
                          'biases': graph.get_tensor_by_name("enc_biases:0")}
                decode = {'biases': graph.get_tensor_by_name("dec_biases:0"),
                ...
                encoded = self._activate(tf.matmul(x, encode['weights']) + encode['biases'], activation)
    ...
    def _activate(self, linear, name):
        if name == 'sigmoid':
